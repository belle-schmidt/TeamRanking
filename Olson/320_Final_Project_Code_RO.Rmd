...
nfl_team_stats_2002_2024 <- read_csv("Olson/nfl_team_stats_2002-2024.csv")
...
#Graphs
library(ggplot2)
week18_compare <- move_3_comparison(team_ranking_week18, Ranking_Least_SquaresWeek18)

ggplot(week18_compare, aes(x = Ranking_1, y = Ranking_2, label = Team)) +
  geom_point() +
  geom_text(nudge_y = 0.5, size = 3) +
  geom_abline(color = "navy") +
  labs(title = "Week 18: Keener's Method Compared to Least Squares Massey's Method",
       x = "Keener Rankings",
       y = "Least Squares Rankings") +
  theme_minimal()

keen_wk18_err <- move_3_comparison(NFL_End_Of_Season_Ranking, team_ranking_week18) |>
  mutate(error = Ranking_2 - Ranking_1)

ggplot(keen_wk18_err, aes(x = reorder(Team, error), y = error)) +
  geom_col(fill = "navy") +
  coord_flip() + 
  labs(title = "Keener's Error Calculated by NFL Rankings at Week 18",
       x = "NFL Teams",
       y = "Ranking Error (positive = overated, negative = underrated)") +
  theme_minimal()

ls_wk18_err <- move_3_comparison(NFL_End_Of_Season_Ranking, Ranking_Least_SquaresWeek18) |>
  mutate(error = Ranking_2 - Ranking_1)

ggplot(ls_wk18_err, aes(x = reorder(Team, error), y = error)) +
  geom_col(fill = "navy") +
  coord_flip() + 
  labs(title = "Least Square's Error Calculated by NFL Rankings at Week 18",
       x = "NFL Teams",
       y = "Ranking Error (positive = overated, negative = underrated)") +
  theme_minimal()
...
keen18_playoff <- team_ranking_week18 |>
  filter(Team %in% NFL_Playoff_Placing$Team) |>
  mutate(model_rank = row_number())
nfl_playoffs <-NFL_Playoff_Placing |>
  mutate(playoff_rank = row_number())
compare_keen18_play <- nfl_playoffs |>
  left_join(keen18_playoff |> select(Team, model_rank), by = "Team") |>
  mutate(err  = playoff_rank - model_rank ) |>
  mutate(overrated = ifelse(err < 0, "Underrated", "Overrated"))

ls18_playoff <- Ranking_Least_SquaresWeek18 |>
  filter(Team %in% NFL_Playoff_Placing$Team) |>
  mutate(model_rank = row_number())
compare_ls18_play <- nfl_playoffs |>
  left_join(ls18_playoff |> select(Team, model_rank), by = "Team") |>
  mutate(err  = playoff_rank - model_rank )|>
  mutate(overrated = ifelse(err < 0, "Underrated", "Overrated"))


nfles_playoff <- NFL_End_Of_Season_Ranking |>
  filter(Team %in% NFL_Playoff_Placing$Team) |>
  mutate(model_rank = row_number())
compare_nfles18_play <- nfl_playoffs |>
  left_join(nfles_playoff |> select(Team, model_rank), by = "Team") |>
  mutate(err  = playoff_rank - model_rank) |>
  mutate(overrated = ifelse(err < 0, "Underrated", "Overrated"))
  
plot_playoff_error <- function(model_df, title_text) {
  model_df |>
    ggplot(aes(x = reorder(Team, err), y = err, fill = overrated)) +
    geom_col() +
    coord_flip() +
    scale_fill_manual(values = c("Overrated" = "darkred", "Underrated" = "darkgreen")) +
    labs(title = title_text,
         x = "NFL Team",
         y = "Playoff Rank − Model Rank",
         fill = "Model Bias") +
    theme_minimal()
}

plot_playoff_error(compare_keen18_play, "Keener Week 18: Playoff Ranking Error")
plot_playoff_error(compare_ls18_play, "Least Squares Week 18: Playoff Ranking Error")
plot_playoff_error(compare_nfles18_play, "NFL End of Season Rankings: Playoff Ranking Error")
```

# Errors for 2024 for each method

```{r}
sum(abs(compare_ls18_play$err))
sum(abs(compare_keen18_play$err))
sum(abs(compare_nfles18_play$err))
```

# Playoff Results for 2023,2022, 2021

```{r}
playoff_result_23 <- data.frame(Team = c("Chiefs", "49ers",
           "Ravens", "Lions",
           "Bills", "Texans", "Buccaneers", "Packers",
           "Browns", "Dolphins", "Steelers",
           "Cowboys", "Eagles", "Rams"))

playoff_result_22 <- data.frame(
  Team = c("Chiefs", "Eagles",
           "Bengals", "49ers",
           "Bills", "Jaguars", "Cowboys", "Giants",
           "Chargers", "Ravens", "Dolphins",
           "Vikings", "Seahawks", "Buccaneers"))

playoff_result_21  <- data.frame(Team = c("Rams", "Bengals",
           "Chiefs", "49ers",
           "Titans", "Bills", "Packers", "Buccaneers",
           "Raiders", "Patriots", "Steelers",
...
```{r}
playoff_error_sum <- function(playoffs, model_rank_df) {
  
  playoffs |>
    left_join(model_rank_df |> select(Team, model_rank), by = "Team") |>
    mutate(err = playoff_rank - model_rank) |>
    summarise(total_error = sum(abs(err), na.rm = TRUE)) |>
    pull(total_error)
}

playoff_23 <- playoff_result_23 |> mutate(playoff_rank = row_number())
playoff_22 <- playoff_result_22 |> mutate(playoff_rank = row_number())
playoff_21 <- playoff_result_21 |> mutate(playoff_rank = row_number())

#2023
keen23 <- team_ranking_week18_23 |>
  filter(Team %in% playoff_23$Team) |>
  mutate(model_rank = row_number())

ls23 <- Ranking_Least_SquaresWeek18_23 |>
  filter(Team %in% playoff_23$Team) |>
  mutate(model_rank = row_number())

nfl23 <- NFL_End_Of_Season_Ranking_2023 |>
  filter(Team %in% playoff_23$Team) |>
  mutate(model_rank = row_number())

keen_err_23 <- playoff_error_sum(playoff_23, keen23)
ls_err_23   <- playoff_error_sum(playoff_23, ls23)
nfl_err_23  <- playoff_error_sum(playoff_23, nfl23)

#2022
keen22 <- team_ranking_week18_22 |>
  filter(Team %in% playoff_22$Team) |>
  mutate(model_rank = row_number())

ls22 <- Ranking_Least_SquaresWeek18_22 |>
  filter(Team %in% playoff_22$Team) |>
  mutate(model_rank = row_number())

nfl22 <- NFL_End_Of_Season_Ranking_2022 |>
  filter(Team %in% playoff_22$Team) |>
  mutate(model_rank = row_number())

keen_err_22 <- playoff_error_sum(playoff_22, keen22)
ls_err_22   <- playoff_error_sum(playoff_22, ls22)
nfl_err_22  <- playoff_error_sum(playoff_22, nfl22)

#2021
keen21 <- team_ranking_week18_21 |>
  filter(Team %in% playoff_21$Team) |>
  mutate(model_rank = row_number())

ls21 <- Ranking_Least_SquaresWeek18_21 |>
  filter(Team %in% playoff_21$Team) |>
  mutate(model_rank = row_number())

nfl21 <- NFL_End_Of_Season_Ranking_2021 |>
  filter(Team %in% playoff_21$Team) |>
  mutate(model_rank = row_number())

keen_err_21 <- playoff_error_sum(playoff_21, keen21)
ls_err_21   <- playoff_error_sum(playoff_21, ls21)
nfl_err_21  <- playoff_error_sum(playoff_21, nfl21)

playoff_error_summary <- data.frame(Season = c(2023, 2022, 2021), Keener = c(keen_err_23, keen_err_22, keen_err_21), Least_Squares = c(ls_err_23, ls_err_22, ls_err_21), NFL = c(nfl_err_23, nfl_err_22, nfl_err_21))

playoff_error_summary
```
