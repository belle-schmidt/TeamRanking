...
Comparison <- function(ranking1, ranking2){
  ...
  for (i in 1:nrow(Ranking_comparison)){
    if (Ranking_comparison[i,1] == Ranking_comparison[i,2]){
      Ranking_comparison$differs[i] = FALSE
    }
    else{
      Ranking_comparison$differs[i] = TRUE
    }
  }
...
# Comparing Week 9 vs Week 18 Ranking
...
```{r}
week9_comp_3 <- move_3_comparison(team_ranking_week9, Ranking_Least_SquaresWeek9)

Week9_Comparisons |> 
  count(differs)

table(week9_comp_3$moved_3_or_less)
```
