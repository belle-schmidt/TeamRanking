...
Least_Squares_data <- Season24 |> 
  select(week, home, away, home_win_margin, score_away, score_home) |> 
...
Least_Squares_data
Least_Squares_For_Keener <- Least_Squares_data |> 
...
A_matrix <- function(input_data) {
  ...
  for (i in 1:(nrow(input_data))){
    if ((input_data$home[i] %in% 1:4 && input_data$away[i] %in% 1:4) |
       ...
       for (j in 1:nrow(input_data)){
         if (input_data$home[i] == input_data$away[j] && input_data$away[i] == input_data$home[j]){
           ...
           print("The two team's are in the same division. The home team in the first game is", row_index)
           col_index <- input_data$away[i]
           print("The away team is", col_index)
    ...
    row_index <- input_data$home[i]
    col_index <- input_data$away[i]
    print("These teams are not in the same division. The home team was", row_index.)
    print("The away team was", col_index)
    ...
    print(message("The aij is", zeros[row_index,col_index]))
       ...
       print(message("The aij is", zeros[row_index,col_index], "The away team is", zeros[col_index,row_index]))
  }
  }
...
Test <- A_matrix(Least_Squares_For_Keener)

Least_Squares_For_Keener
